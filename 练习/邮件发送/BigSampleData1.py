...
def downloadFile():
    ...
    LocalPath = pathlib.Path('D:\\no_delete_task_data\\BigSampleData\\'+getYesterday()+'.tgz')
    request.urlretrieve(url,LocalPath)

# print(1, s)

def send_mail(sender = 'ziwei.zhang@xor-media.tv',receivers = ['zhangjian03@bgctv.com.cn,niechenlei@bgctv.com.cn,dingganting@bgctv.com.cn,weimingkai@bgctv.com.cn,wangying1@bgctv.com.cn,shengting.luo@xor-media.tv,ziwei.zhang@xor-media.tv'],sender_password = 'ICE0JCIJBdP5qvtD',smtp_sender = 'mail.xor-media.tv', subject = '数据汇总：'+getYesterday()):
...
# 构造附件1,传送当前目录下的test.txt文件
    att1 = MIMEText(open('D:\\no_delete_task_data\\BigSampleData\\'+getYesterday()+'.tgz','rb').read(),'base64','utf8')
